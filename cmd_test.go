...
func TestOutCmd(t *testing.T) {
	cmd := OutCmd(context.Background(), os.Args[0], "-printArgs", "foo", "bar")
	out, err := cmd("baz", "bat")
	if err != nil {
		t.Fatal(err)
	}
...
func TestExitCode(t *testing.T) {
	ran, err := Exec(context.Background(), nil, nil, nil, os.Args[0], "-helper", "-exit", "99")
	if err == nil {
...
func TestEnv(t *testing.T) {
	...
	out := &bytes.Buffer{}
	ran, err := Exec(context.Background(), map[string]string{env: "foobar"}, out, nil, os.Args[0], "-printVar", env)
	if err != nil {
...
func TestNotRun(t *testing.T) {
	ran, err := Exec(context.Background(), nil, nil, nil, "thiswontwork")
	if err == nil {
...
func TestAutoExpand(t *testing.T) {
	...
	s, err := Output(context.Background(), "echo", "$MAGE_FOOBAR")
	if err != nil {
		t.Fatal(err)
	}
	if s != "baz" {
